// V1.5.0 vom 23.4.2020 - https://github.com/Pittini/iobroker-Fensterauswertung - https://forum.iobroker.net/topic/31674/vorlage-generisches-fensteroffenskript-vis
...
const WindowIsOpenWhen = ["true", "offen", "open", "opened", "1"]; // Hier können eigene States für offen angegeben werden, immer !!! in Kleinschreibung
...
const WindowIsTiltedWhen = ["tilted", "gekippt", "2"]; // Hier können eigene States für gekippt angegeben werden, immer !!! in Kleinschreibung
...
function init() {
    ...
    for (let x = 0; x < RoomList.length; x++) { //Messaging DPs einlesen
        ...
        if (logging) log("x=" + x + " SendWarnMsg=" + SendWarnMsg[x] + " SendVentMsg=" + SendVentMsg[x] + " SendOpenCloseMsg=" + SendOpenCloseMsg[x]);
    };
...
function SimplyfyWindowStates(x) { //Die verschiedenen Gerätestates zu open, close oder tilted vereinfachen
    ...
    else if (WindowIsTiltedWhen.indexOf(SensorVal[x]) != -1) { // Suche in Fenstergekipptarray, wenn gefunden, Status auf tilted setzen
        SensorVal[x] = "tilted";
    };

    if (SensorVal[x] != "open" && SensorVal[x] != "closed" && SensorVal[x] != "tilted") { // Suche in Fensteroffenarray und Fenstergeschlossenarray, wenn nirgends gefunden, Status auf closed setzen und Logwarnung ausgeben
    ...
    else if (WindowIsTiltedWhen.indexOf(SensorOldVal[x]) != -1) {
...
function CreateTrigger() {
    ...
    for (let x = 0; x < RoomList.length; x++) { //Alle Räume durchlaufen
        on(praefix + RoomList[x] + ".VentWarnTime", function (dp) { //Trigger für VentwarnTime in Schleife erstellen
